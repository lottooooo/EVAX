...
### PART 10A1 Overlap between Qualtrics survey
##Namely between:
## 1. EVAX_INS and EVAX_Main
####### REMOVE ALL OBJECTS IN WORKPLACE ########
rm(list=ls())
...
# Read the first column as a character vector
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10a/10A1")
varlist_INS <- read_excel("varlist_INS.xlsx", sheet = "MAIN", col_names = FALSE) %>%
  pull(1) %>%                      # take first column to ensure it stays vector rather df
  as.character() %>%               # make sure it's a character vector
  unique()                         # (optional) remove duplicates

MAIN_residentINS_clean_2 <- MAIN_residentINS_clean_1 %>%
  select(
    any_of(names(residentMAINIADL_clean)),  # Rule 1: keep shared columns
    any_of(varlist_INS)               # Rule 2: include extra columns from varlist
    # (these may not appear in residentSM_clean, but should still be kept if present)
  )


##3. Filter MAIN so it only include PIDs that are in INS
MAIN_residentMAINIADL_clean_1 <- residentMAINIADL_clean %>%
  filter(PID %in% residentINS_clean$PID)

##4 Only include matching varnames in new MAIN df + manually add matching var from Resident_R2 Variable Matching to R1.xlsx
# Read the first column as a character vector
#Same as previous MAIN sheet as all varname same between INS and MAIN
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10a/10A1")
varlist_INS <- read_excel("varlist_INS.xlsx", sheet = "MAIN", col_names = FALSE) %>%
  pull(1) %>%                      # take first column to ensure it stays vector rather df
  as.character() %>%               # make sure it's a character vector
...
MAIN_residentMAINIADL_clean_2 <- MAIN_residentMAINIADL_clean_1 %>%
  select(
    any_of(names(residentINS_clean)),  # Rule 1: keep shared columns
    any_of(varlist_INS)               # Rule 2: include extra columns from varlist
    # (these may not appear in residentSM_clean, but should still be kept if present)
  )

### Download SM_residentINS_clean_2 & SM_residentSM_clean_2 to manually compare
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10a/10A1")
write_xlsx(MAIN_residentINS_clean_2, "MAIN_residentINS_clean_2.xlsx")
write_xlsx(MAIN_residentMAINIADL_clean_2, "MAIN_residentMAINIADL_clean_2.xlsx")

###Further checking which varnames are not empty/missing/unknown values in EVAX_INS
#List the INS values that's NOT included in matching to MAIN
allowed <- c("", "Unknown", "不清楚", "不清楚/暫未發現")

#create a list of INS colnames/vars that have items not in ALLOWED i.e. actual values 
...
# Create a named list: each element = the PIDs having disallowed values for that column
...
invalid_pid_list <- map(cols_filtered, function(colname) {
  ...
  MAIN_residentINS_clean_2 %>%
    filter(
      # Keep rows NOT in allowed values (after converting to character)
...
# Name each list element by its column name
...
##Create new DF
##Remarks: Pasting cols_filtered variables in both INS and MAIN to see whether they match by PID
# 1. All PIDs that have any non-ALLOWED value in INS for any of the selected columns
pids_to_keep <- invalid_pid_list %>%
  unlist() %>%      # flatten list -> vector
  unique()          # remove duplicates

##2. build MAIN side columns (suffix_MAIN)
...
###cols_filtered   # e.g. "LTI", "LTI_type_1", ...

# Construct new column order: PID, name_CHI, then MAIN/INS pairs
ordered_cols <- c(
  "RCHEID",
  "PID",
  as.vector(rbind(paste0(cols_filtered, "_MAIN"), paste0(cols_filtered, "_INS")))
)
# Apply the new order (keeping any missing columns automatically ignored)
MAIN_residentMAINIADL_clean_3 <- MAIN_residentMAINIADL_clean_3 %>%
  select(any_of(ordered_cols))

### Download SM_residentINS_clean_2 & SM_residentSM_clean_2 to manually compare
##still contains duplicates PID cus of misspelling of chinese names
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10a/10A1")
write_xlsx(MAIN_residentMAINIADL_clean_3, "MAIN_residentMAINIADL_clean_3.xlsx")
## final DF only contains matching colnames in both MAIN and INS where both have valid values (not NA or blank)
##e.g. if INS is totally blank but MAIN have value, that column would not be included...
...
### Part 10B Within-Round Harmonisation of R1 INS & MAIN

###Part 10B1 -  WITHIN DECISION MATRIX (only parent)
#requires 1 dataset (overlap_R1_wide) and 2 metadata (R1 Overlay map, sheet = overlapping vars & within_decision_matrix)

#1. read R1 "clean" dataset for only parent vars and all of their breakdown var (including non-overlaps)
##read R1 dataset
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B1")
R1_wide <- read_excel("combined_R1_Res_3.xlsx")

#read overlap var dataset to select overlapping PIDs only
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B1")
overlap_PIDs <- read_excel("MAIN_residentMAINIADL_clean_finalcompare.xlsx")

##read var map amd within_code
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B1")
var_map <- read_excel("R1_overlay_map_23122025.xlsx", sheet = "Overlapping_vars")
within_code <- read_excel("R1_overlay_map_23122025.xlsx", sheet = "within_decision_matrix")

##Step 1 - Filter your overlay map to parents var ONLY
...
#filter var_map to only include parent_vars
...
#1. # Helper: split "a; b; c" into c("a","b","c"), safely
split_semicol <- function(x) {
  if (is.na(x) || trimws(x) == "") return(character(0))
  str_split(x, "\\s*;\\s*", simplify = FALSE)[[1]] |> trimws()
}

#2. # Build a per-parent-concept lookup table
parent_cols <- parent_var_map %>%
  transmute(
    concept = Concept,
    parent_main = first_main,   # e.g. "LTI_MAIN"
    parent_ins  = first_ins,    # e.g. "LTI_INS"
    breakdown_base_main = map(breakdown_main, split_semicol), # e.g. c("LTI_type_1","LTI_type_2",...)
    breakdown_base_ins  = map(breakdown_ins,  split_semicol)
  ) %>%
  mutate(
    # translate base names -> actual wide column names
...
#Step 3 - compute Parent_class and breakdown_class in overlap_R1_wide df
#1. Define classifcaition rule once - based on within-decision_matrix
#For parents
blank_vals   <- c("", NA)
unknown_vals <- c("Unknown", "不清楚", "不詳", "不知道")  # add your variants
pos_vals     <- c("有", "是", "Yes", "Y", "1")                 # adjust
neg_vals     <- c("無", "無病徵", "否", "No", "N", "0")                  # adjust


#Classify parent and breakdown
...
for (i in seq_len(nrow(parent_cols))) {
  ...
  # parent classes always computable
  df <- df %>%
    mutate(
  ...
  # breakdown signal MAIN
  if (length(b_main) > 0) {
    df <- df %>%
      mutate(
        !!paste0(concept, "_breakdown_signal_MAIN") := if_else(
          if_any(all_of(b_main), is_breakdown_positive),
          "Valid Positive", "Blank"
        )
      )
  } else {
  ...
  # breakdown signal INS
  if (length(b_ins) > 0) {
    df <- df %>%
      mutate(
        !!paste0(concept, "_breakdown_signal_INS") := if_else(
          if_any(all_of(b_ins), is_breakdown_positive),
          "Valid Positive", "Blank"
        )
      )
  } else {
...
##quick check for the new cols added for each parent concepts
# should see 2 parent class col and 2 breakdown signal col for each parent var
names(overlap_R1_wide)[grepl("^LTI_|^HOS_SYMP_|^LCOV_", names(overlap_R1_wide))]

##Step 4 - Apply within decision matrix (using within_code df)
#1. Apply decision matrix for all parent concepts (INS + MAIN)
df <- overlap_R1_wide
...
for (i in seq_len(nrow(parent_cols))) {
  ...
  # column names already created in Step 3
  ...
  # --- sanity check: warn if any unmatched combos (Within_state becomes NA) ---
...
##After running, for each concept (i.e. LTI) - there is  8 new cols
#i.e.
# LTI_Final_parent_output_MAIN
# LTI_Within_state_MAIN
# LTI_outcome_type_source_MAIN
# LTI_parent_final_source_MAIN
...
#is concept_parent_class_MAIN = concept_Final_parent_output_INS
#is concept_parent_class_MAIN = concept_Final_parent_output_MAIN

check <- check %>%
  mutate(
    # A) Did within logic change the parent class? (MAIN)
    ...
    # A) Did within logic change the parent class? (INS)
...
#check if any concept_parent_same_dataset have any FALSE! FALSE = inconsistency
...
#Step 5 - Create the final parent value col (concept_parent_final_value_MAIN) on the within decision matrix
##FINAL parent final value = concept_parent_final_value_MAIN

# Canonicalize parent values (define AFTER rm(list=ls()))
...
##save resolved within df as excel
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B1")
write_xlsx(overlap_R1_wide, "overlap_R1_wide.xlsx")

##OKAY, now within dataset coherency issues are resolved we are consistent within dataset
##i.e. all parent variables matches breakdown
...
#requires 1 dataframe (overlap_R1_wide) and 2 metadata (R1_overlay_map, sheet = overlapping_vars & between_decision_matrix)
#1. read R1 "clean" dataset for all roles
##read R1 dataset
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B2")
overlap_R1_wide <- read_excel("overlap_R1_wide.xlsx")

##read var map amd between_code
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B2")
var_map <- read_excel("R1_overlay_map_23122025.xlsx", sheet = "Overlapping_vars")
between_code <- read_excel("R1_overlay_map_23122025.xlsx", sheet = "between_decision_matrix")

#2. role mapping: adjust if your hierarchy_type uses different labels
between_map <- var_map %>%
...
#Parents var - use final cols from overlap_R1_wide
...
#4. Step 1 - Extract between "long" working table (PID x concept)
...
between_long <- pmap_dfr(
  between_map,
  ~ extract_between_one(overlap_R1_wide, ..1, ..2, ..3, ..4)
)

#5. Step 2 - Classify MAIN/INS values into between classes to long df
#use the same classifiers as part 10c1 (within matrix) as parents already standardised
blank_vals   <- c("", NA)
...
#6. Step 3 - Apply between decision matrix to long df
...
between_res <- between_long %>%
  ...
  left_join(
    between_lookup,
    by = c("Role" = "Role", "MAIN_class", "INS_class")
  )

##QC - check unmatched (not included in between decision matrix) 
between_res %>%
  filter(is.na(final_source)) %>%
  count(Role, MAIN_class, INS_class, sort = TRUE) #RESULT: ALL matched

#7. Step 4 -  Build final chosen value for between matrix
between_res <- between_res %>%
  mutate(
    final_value_between = case_when(
      final_source == "MAIN" ~ MAIN_value,
      final_source == "INS"  ~ INS_value,
      final_source == "BOTH" ~ MAIN_value,  # they should match
      TRUE ~ NA
    )
  )
##What df look like now:
##have resolved between INS/MAIN inconsistencies for every concept except 
##those in matrix that say "Needs conflict logic? = Yes)

##8. Step 5. Handle "needs conflict logic? = yes" rows
##safe pick final value using final_source
resolve_between_conflict <- function(df) {
  df %>%
    mutate(
      ...
      # choose final value
      final_value_between = case_when(
        Rule %in% c("B5", "C5") ~ "NA", #for BLANK/BLANK
        final_source == "MAIN" ~ MAIN_value,
        final_source == "INS"  ~ INS_value,
        final_source == "BOTH" ~ MAIN_value,  # should be the same as INS_value in BOTH cases
        TRUE ~ NA
      ),
      
      # optional: create an audit note for the "Yes" cases
...
#apply function to between_res
between_res <- resolve_between_conflict(between_res)

##QC ## no unresolved final values for merging between INS/MAIN
between_res %>%
...
final_between_wide <- between_res %>%
  ...
  # if you have duplicates per PID×concept, resolve them first
  group_by(PID, RCHEID, final_name_CHI, concept) %>%
  summarise(
    final_value_between = dplyr::first(final_value_between),
    .groups = "drop"
  ) %>%
  # pivot to wide
  pivot_wider(
...
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B2")
write_xlsx(final_between_wide, "final_between_wide.xlsx")

#######################################################################
####### REMOVE ALL OBJECTS IN WORKPLACE ########
rm(list=ls())

## PART 10B3 - Joining final resolved df for INS/MAIN back into the OG dataset (final)

setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B3")
R1_wide <- read_excel("combined_R1_Res_3.xlsx")
final_between_wide <- read_excel("final_between_wide.xlsx")
var_map <- read_excel("R1_overlay_map_23122025.xlsx", sheet = "Overlapping_vars")

#Step 1: Join final_between_wide back to R1
# keep only PID + final_* columns for joining
...
#Step 2: Build a concept - uses var_map to know which raw columns correspond to which concept
#uses var_map to know which raw columns correspond to which concept
#rationale: easier to use concept (i.e. LTI) than 3 cols (INS/MAIN/overlapped) for one col
to_main_raw <- function(x) paste0("ResidentMAINIADL.", sub("_MAIN$", "", as.character(x)))
to_ins_raw  <- function(x) paste0("ResidentINS.",     sub("_INS$",  "", as.character(x)))
...
## Step 3: Create a single "final merged" column per concept
##i.e. final column is LTI_FINAL rather than LTI_MAIN, LTI_INS, LTI_overlap...
#falls back to raw MAIN/INS for non-overlapping PIDs

#as coalesce() treats NA as missing, not "", so we convert "" → NA first.
...
for (i in seq_len(nrow(xwalk))) {
  ...
  final_col <- paste0("final_", concept)      # from final_between_wide
  
  # only do this if the columns exist (safety)
...
##QC = ALL PASSED
#QC1 - row count + PID uniqueness
# 1) PID uniqueness preserved
stopifnot(dplyr::n_distinct(R1_full$PID) == dplyr::n_distinct(R1_wide$PID))

# 2) No duplicated PID created
R1_full %>% count(PID) %>% filter(n > 1)


#QC2 - PID set unchanged
setequal(R1_full$PID, R1_wide$PID)

# If FALSE, inspect differences:
setdiff(R1_wide$PID, R1_full$PID)   # lost
setdiff(R1_full$PID, R1_wide$PID)   # added

##QC3 - Ensure did not overwrite raw MAIN/INS cols
...
#QC4 - verify that every concept in xwalk$concept (overlapping vars) got a final column
expected_final <- paste0("final_", xwalk$concept)
setdiff(expected_final, names(R1_full))

##FINAL RESOLVED INS/MAIN R1 DF
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10b/Part 10B3")
write_xlsx(R1_full, "R1_full.xlsx")
...
#Objective:
# - Build SM overlay map (excluding ResidentADD2.*)
# - Create unified final columns named FIN_<concept>
#Hierarchy:
#   ResidentMINI > ResidentSM > final_<concept> (from Part 10c INS/MAIN) >
#   ResidentMAINIADL > ResidentINS > ResidentVISVH > ResidentMC
#Output columns:
#   FIN_<concept> and FIN_<concept>_source

#0. Load inputs
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10c")
R1_full <- read_excel("R1_full.xlsx")
...
priority_prefix <- c(
  "ResidentMINI",
  "ResidentSM",
  "final",             # treat final_ as a "source" conceptually in map
  "ResidentMAINIADL",
  "ResidentINS",
  "ResidentVISVH",
  "ResidentMC"
...
order_by_priority <- function(cols, priority_prefix) {
  ordered <- unlist(lapply(priority_prefix, function(pfx) {
    if (pfx == "final") {
      # "final_<concept>" is not prefix-dot format, handle separately later
...
# For each SM concept, find which columns exist across sources that share the same concept name
concept_sources <- lapply(sm_concepts_all, function(concept) {
  
  # all dot-style sources e.g. ResidentSM.X, ResidentINS.X, ResidentVISVH.X
  cols_dot <- grep(
  ...
  # EXCLUDE ResidentADD2.*
  cols_dot <- cols_dot[!startsWith(cols_dot, "ResidentADD2.")]
  
  # include final_<concept> if it exists
...
sm_overlay_map <- tibble(concept = sm_concepts_all) %>%
  mutate(
    ...
    sources = map(cols_all, ~ {
      # label sources in a readable way
...
# Save overlay map
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10c")
write_xlsx(sm_overlay_map, "SM_overlay_map.xlsx")


# 3. PHASE 1: Build FIN_<concept> (only for SM concepts)
df <- R1_full

# You said: "create this for all SM vars using SM overlay map"
# => only create FIN_ columns for concepts present in SM (excluding meta)
...
for (concept in concepts_to_make_fin) {
  ...
  sm_col    <- paste0("ResidentSM.", concept)
  old_final <- paste0("final_", concept)              # from Part 10c (INS/MAIN overlap)
  ...
  # Hierarchy (only among these)
  df[[out_col]] <- pick_first_nonblank(df, c(mini_col, sm_col, old_final, main_col, ins_col, vis_col, mc_col))
  
  # Audit: which column won
  df[[src_col]] <- NA_character_
  
  # MINI
  if (mini_col %in% names(df)) {
    hit <- !is.na(blank_to_na(df[[mini_col]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[mini_col]]))
    df[[src_col]][hit] <- mini_col
  }
  
  # SM
  if (sm_col %in% names(df)) {
    hit <- is.na(df[[src_col]]) &
      !is.na(blank_to_na(df[[sm_col]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[sm_col]]))
  ...
  if (old_final %in% names(df)) {
    hit <- is.na(df[[src_col]]) &
      !is.na(blank_to_na(df[[old_final]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[old_final]]))
  ...
  if (main_col %in% names(df)) {
    hit <- is.na(df[[src_col]]) &
      !is.na(blank_to_na(df[[main_col]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[main_col]]))
  ...
  if (ins_col %in% names(df)) {
    hit <- is.na(df[[src_col]]) &
      !is.na(blank_to_na(df[[ins_col]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[ins_col]]))
  ...
  if (vis_col %in% names(df)) {
    hit <- is.na(df[[src_col]]) &
      !is.na(blank_to_na(df[[vis_col]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[vis_col]]))
  ...
  if (mc_col %in% names(df)) {
    hit <- is.na(df[[src_col]]) &
      !is.na(blank_to_na(df[[mc_col]])) &
      (blank_to_na(df[[out_col]]) == blank_to_na(df[[mc_col]]))
...
# Save FINAL DF (ALL overlapping vars-pids are resolved)
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10c")
write_xlsx(R1_full_final, "R1_full_final.xlsx")
...
### #PART 10D JOINING ALL ROUNDS [R1, R2.1, R2.2 & R3] - (Prep for Part 10e Phase 1)
#WITH SPECIMEN LOG
setwd("C:/Users/OrielTsao/Desktop/COVID-19 RCHEs/DATA/10d/ALL ROUNDS/Resident")

## 1. Read the R2 survey files (no col names, because first 3 rows are special)
R1_raw <- read_excel("R1_full_final.xlsx", col_names = T)
R2_raw <- read_excel("combined_R2_Res_3.xlsx", col_names = T)
R3_raw <- read_excel("combined_R3_Res_3.xlsx", col_names = T)
...
#Prep 4: Remove E101-1-003 from R1 resident as no consent (already accounted for in Round blood count)
spec_log_clean <- spec_log_clean %>% filter(PID != "E101-1-003") # total R1 res blood = 689

#Prep 5: # Standardize PID format in both dataframes
...
#4. Create a new R1 DF from blood_df & consent_df which for the SAME PID, SELECT 1 ROW FROM SPECIMEN LOG WITH CLOSEST DATE & AFTER CONSENT
### THIS DF include all even those filter out by cdate >= consent_date (denoted by NA)

#Step 1: filter out R2 and R3 (ONLY R1)
...
finalwmissing_df <- finalwmissing_df %>%
  mutate(cdate = if_else(is.na(cdate), 
                                as.Date("9999-12-31"), cdate))


#finalwmissing_df are all blood PID with blood data nearest to its consent date (including 4 NA)
#QC does finalwmissing contains all PIDs from specimen_log
uniqueblood <- blood_df %>% 
  distinct(PID, .keep_all = TRUE)

identical(sort(uniqueblood$PID), sort(finalwmissing_df$PID)) #IDENTICAL = TRUE


# STEP 5 (AFTER PASS QC): CREATE DF THAT ONLY CONTAINS THE BLOOD DATA FROM finalwmissing_df
R1_blood_data <- finalwmissing_df %>% 
  select(subid1:PID)

#5. Join R1_blood_data to R2 & R3 blood data from spec_log
#Step 1:Additional prep
#Prep 1: create a RCHEID column from subid1
spec_log_clean$RCHEID <- spec_log_clean$subid1
R1_blood_data$RCHEID <- R1_blood_data$subid1

#Prep 2:  create a round column in spec_log
spec_log_clean <- spec_log_clean %>%
  mutate(round = case_when(
    # 1. Check if the year is 2022
    str_starts(as.character(cdate), "2022") ~ "1",
    
    # 2. If not 2022, check EVAX_Status for Round2 or Round3
    EVAX_Status == "Round2" ~ "2",
    EVAX_Status == "Round3" ~ "3",
    
    # 3. Round 1 if none of the above are true
    TRUE ~ "1"
  ))
...
#Step 2: QC R2 & R3 spec_log blood data
...
#Step 3: QC R2 & R3 blood - all have consent
...
##8. Make sure every column has RCHEID
ALLRound <- ALLRound %>%
...
write.xlsx(ALLRound, 
           file = "ALLRound_Resident.xlsx",
           colNames = TRUE,
           rowNames = FALSE)

## save All_blood_date (chosen from spec_log) into excel
write.xlsx(ALL_blood_data, 
           file = "ALL_blood_data.xlsx",
           colNames = TRUE,
           rowNames = FALSE)
...
length(unique(R1$PID)) # 1190 #1189 if remove ENA-1-NA
